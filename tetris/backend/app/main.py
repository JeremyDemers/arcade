...
from .schemas import GoogleExchangeRequest, AuthResponse, ScoreCreate, ScoreResponse, StatsResponse, UserResponse
...
app = FastAPI(title="Arcade Tetris API", version="0.2.0")
app.add_middleware(
    CORSMiddleware,
...
@app.get("/games/tetris/leaderboard", response_model=list[ScoreResponse])
def leaderboard() -> list[ScoreResponse]:
    with get_connection() as connection:
        rows = connection.execute(
            """
            SELECT scores.id, users.username, scores.score, scores.level, scores.lines, scores.created_at
...
@app.post("/games/tetris/scores", response_model=ScoreResponse, status_code=status.HTTP_201_CREATED)
def save_score(payload: ScoreCreate, user: sqlite3.Row = Depends(current_user)) -> ScoreResponse:
    with get_connection() as connection:
        cursor = connection.execute(
            "INSERT INTO scores (user_id, game_slug, score, level, lines) VALUES (?, ?, ?, ?, ?)",
            (user["id"], GAME_SLUG, payload.score, payload.level, payload.lines),
        )
...
@app.get("/games/tetris/stats", response_model=StatsResponse)
